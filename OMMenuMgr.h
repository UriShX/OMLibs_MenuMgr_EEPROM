...
#ifndef OM_EXIT_TIMER
	#define OM_EXIT_TIMER 3000
#endif
...
//#define ENC_K  _key//EncoderVal


/*
typedef struct EncoderVal {
	static volatile int8_t* keyValue;
}_key;
*/
/** Select-Type Item
...
enum  { MENU_ANALOG, MENU_DIGITAL, MENU_ENCODER };
...
/** OpenMoCo Menu Manager Class
 ...
 const MENU_ITEM item_foo  = { {"Foo Edit"},    ITEM_VALUE,  0,        MENU_TARGET(&value_foo) };
 @endcode
 ...
 const MENU_VALUE value_foo = { TYPE_BYTE,       100,   0,     MENU_TARGET(&foo) };
 ...
 @code
 const MENU_LIST const root_list[] = { &item_foo, &item_bar };

 const MENU_ITEM menu_root = { {"Root Menu"},  ITEM_MENU,  MENU_SIZE(root_list),  MENU_TARGET(&root_list) };
 @endcode
 ...
 const MENU_SELECT state_select = { &myVar,           MENU_SELECT_SIZE(state_list),   MENU_TARGET(&state_list) };

 const MENU_VALUE  value_sel = { TYPE_SELECT, 0, 0, MENU_TARGET(&state_select) };
 const MENU_ITEM   item_sel  = { {"Select It"}, ITEM_VALUE, 0, MENU_TARGET(&value_sel) };
 @endcode
 ...
 const MENU_FLAG    my_flag    = { 3, &theseFlags };
 const MENU_VALUE   my_value   = { TYPE_BFLAG, 0, 0, MENU_TARGET(&my_flag) };
 const MENU_ITEM    my_item    = { {"Flag Edit"}, ITEM_VALUE, 0, MENU_TARGET(&my_value) };
 @endcode
...
public:
	...
	void setEncoderPins(const int p_pins[3][2]);
	...
	//volatile static int _key;
	
	static volatile int _checkEncISR(int8_t x);
	
	volatile int8_t _key;
